...
else
{
    ...
    foreach($tpcId in $tpcIds){
        ...
        Write-Host "Destroying WI /w Title containing `"$titleText`"" -foregroundcolor Red
        ...
        { 
            ...
            $matches = $results | ? {$_.Title.ToLower().Contains($titleText)}
            $idList = $matches | % {$_.Id}
            ...
            Write-Host "$($tp.Name) - WI Destroyed: $($idList.Count)"
        }
